...
class MainActivity: FlutterActivity() {
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        // Setup method channel for background service
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "tm.ranjith.trackmywork/background").setMethodCallHandler { call, result ->
            when (call.method) {
                "setupHeartbeat" -> {
                    val intervalMinutes = call.argument<Int>("intervalMinutes") ?: 5
                    setupHeartbeat(intervalMinutes)
                    result.success(true)
                }
                "startForegroundService" -> {
                    val activityName = call.argument<String>("activityName") ?: "Unknown Activity"
                    val iconName = call.argument<String>("iconName") ?: "access_time"
                    startTimeTrackingService(activityName, iconName)
                    result.success(true)
                }
                "updateForegroundService" -> {
                    val activityName = call.argument<String>("activityName") ?: "Unknown Activity"
                    val formattedTime = call.argument<String>("formattedTime") ?: "00:00:00"
                    val iconName = call.argument<String>("iconName") ?: "access_time"
                    updateTimeTrackingService(activityName, formattedTime, iconName)
                    result.success(true)
                }
    ...
    private fun openBatteryOptimizationSettings() {
        val intent = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            // For older devices, just open app settings
    ...
    private fun setupHeartbeat(intervalMinutes: Int) {
        ...
        Log.d("MainActivity", "Heartbeat scheduled every $intervalMinutes minutes")
    }

    private fun startTimeTrackingService(activityName: String, iconName: String) {
        ...
        Log.d("MainActivity", "Started time tracking service for: $activityName")
    }

    private fun updateTimeTrackingService(activityName: String, formattedTime: String, iconName: String) {
        val serviceIntent = Intent(this, TimeTrackingService::class.java).apply {
    ...
    private fun stopTimeTrackingService() {
        ...
        Log.d("MainActivity", "Stopped time tracking service")
    }
